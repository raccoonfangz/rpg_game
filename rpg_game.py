...
def opp_attack(opponent):
    return random.randint(*opponent["Attack"])
...
opponent_list = {  # 50% to heal to full hp while under 15% zombies health
    ...
    "Knight": {  # 10% dmg reduiction from player
        "Base_Health": 30,
        "XP": 35,
        "Attack": (1, 14),
        "Reward": (10, 25),
...
flee_events = {
    ...
    "lost": {
        "message": "You were running so fast you're now lost in the forest\nGetting back on path has cost you 10 health",
        "health_penalty": 10
    },
    "unlucky": {
...
def select_weapon():
    ...
    while game_state['GAME_RUNNING']:
        ...
        print("-" * 64)
        print(
            "Finest selection of weapons:"
            "\n1. Wooden sword | 3-11 dmg - Basic safe choice"
            "\n2. Wooden spear | 5-8  dmg - Another classic"
            "\n3. Glass shard  | 0-3  dmg - Why would you pick that?"
            "\n4. Magic coin   | 0 or 100 dmg - 10% chance to land"
        )
        if "steelsw" in game_state['BOUGHT_WEAPONS']:
            print("5. Steel sword  | 10-17 dmg - Bought")
        if "steelax" in game_state['BOUGHT_WEAPONS']:
            print("6. Steel axe    | 13-20 dmg - Bought")
        if "ironmace" in game_state['BOUGHT_WEAPONS']:
            print("7. Iron mace    | 18-24 dmg - Bought")
        if "shiny infinity blade" in game_state['META_ITEMS']:
            print("8. Shiny infinity blade | 60-120dmg - Bought")
        if "world splitter" in game_state['META_ITEMS']:
            print("9. World splitter | 200-400dmg - Bought")
        print("-" * 64)
        ...
        if weapon_choice in weapon_map:
            ...
            if selected_key in inventory_items:
                if (
                    ...
                    clear()
                    print("You don't own that weapon yet.")
                    time.sleep(1.2)
            else:
                clear()
...
def select_enchantment():
    ...
    while game_state['ENCHANTMENT_SELECTION']:
        ...
        if selection in ("vampirism", "1"):
            ...
            print("\nInvalid enchantment choice. Try again.")
            time.sleep(1.2)
            clear()
        time.sleep(1.2)
...
def shrine_event():
    ...
    if option == "y" or option == "yes":
        ...
        if game_state["COIN_BALANCE"] >= 15:
            ...
            clear()
            print("\nYou can't afford it man. Here I'll give you 5c I feel bad for you")
...
def combat_mechanic():
    ...
    while opponent["Health"] > 0:
        ...
        if choice == "1" or choice == "attack":  # Attack
            ...
            print(f"You defended! Damage reduced to {opp_damage}.")
        ...
        elif choice == "4" or choice == "flee":  # Flee
            time.sleep(0.5)
            clear()
            flee_mechanic()
            time.sleep(1)
...
def lvl_rewards():
    ...
    print(
        "Rewards roadmap:"
        "\nLVL - 5: 100c | Higher bets unlocked"
    ...
    if level >= 5 and "lvl5" not in game_state['CLAIMED_LVL']:
        ...
        if opt == "y" or opt == "yes":
            ...
            game_state['TOTAL_COINS_EARNED'] += 100
    ...
    if game_state.get('ALCOHOL_CONSUMED', 0) <= 10:
        if choice == "1" or choice == "beer":
            if game_state["COIN_BALANCE"] >= 10 and game_state['PLAYER_HEALTH'] > 5:
                ...
                clear()
                print("Not enough coins or drinking would kill you")
                time.sleep(1)
                clear()
                tavern()
...
def meta_shop():
    ...
    print(f"{'No':<3} {'Item':<40} {'Cost':>10}")
    print("-" * 64)
    print(f"{'1.':<3} {'Survivor perk - survive one lethal damage once':<40} {'300p':>10}")
...
while game_state['GAME_RUNNING'] == True:
    ...
    # player info
        ...
        input("\nPress Enter to return to main menu...")
    # level
    elif command == "progress" or command == "4":
        time.sleep(0.01)
        clear()
        progress()
    # rest
